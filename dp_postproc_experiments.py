...
for i in range(len(gamma_list)):
    for j in range(num_rounds):
        errtilde[i,j], unftilde[i,j] = dpalgo1.dp_postproc.eq_odds(Yhat, A, Y, epsilon, gamma_list[i], beta)
...
plt.title('dp_postprocessing, lawschool, epsilon = %i' %epsilon)
plt.savefig('dp_postproc_lawschool.png')
plt.show()